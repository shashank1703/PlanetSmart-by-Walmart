...
//       setMessages((prevMessages) => [
//         ...prevMessages,
//         { type: "assistant", content: "Sorry, I'm having trouble connecting right now. Please try again later." },
//       ])
//     }
//   }

//   const handleSuggestionClick = async (suggestion) => {
//     const userMessage = { type: "user", content: suggestion.text }
//     setMessages((prevMessages) => [...prevMessages, userMessage])
...
//           "Content-Type": "application/json",
//         },
//         body: JSON.stringify({ message: suggestion.text }),
//       })
...
//       ])
//     } catch (error) {
//       console.error("Error sending suggestion to Gemini API:", error)
//       setMessages((prevMessages) => [
//         ...prevMessages,
//         { type: "assistant", content: "Sorry, I'm having trouble processing that suggestion. Please try again later." },
//       ])
//     }
//   }
...
const BACKEND_URL = "https://ecobot-backend-1-u331.onrender.com/api/ask-eco"
...
export default function AIEcoAssistant() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const data = await response.json()
      const reply = data.answer || "⚠️ Sorry, I couldn't process your request."
  ...
  const handleSuggestionClick = async (suggestion) => {
    ...
    try {
      const response = await fetch("https://ecobot-backend-1-u331.onrender.com", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ question: suggestion.text }),
      })
